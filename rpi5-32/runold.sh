...
#for files in 1000; do

	for p in $PARTITIONS; do

		for t in join index count pgbench star; do
		#for t in join; do

			# partitions patch
			for partitions in yes no; do

				for glibc in yes no; do

					# fastpath lock groups
					for groups in 32 0 1 1024 8 4096 512 32768 128; do
					#for groups in 32 0; do

						RUN=$t-$partitions-$glibc-$groups-$files-$p-$(date +%s)
						LOGDIR=$LOGS/$RUN
						...
						echo "shared_buffers = 1GB" >> $DATADIR/postgresql.auto.conf
						...
						./run-$t.sh "$MACHINE $BRANCH $files $partitions $glibc $groups" $LOGDIR $RUNS $DURATION "$CLIENTS" "$p" > $RESULTDIR/$t.csv
