					...
					for glibc in no yes; do
						...
						for pool in no yes; do
							...
							for groups in 64 0 1 512 8 256 4096; do
								...
								BRANCH="pg-partitions-$partitions-groups-$groups-pool-$pool-guc"
								...
								if [ ! -d "$BUILDS/$BRANCH" ]; then
									...
									if [ "$groups" != "0" ]; then
										cat $PATCHES/v20240905-0001-v1.patch | patch -p1
										cat $PATCHES/v20240905-0002-rework.patch | patch -p1
										cat $PATCHES/v20240905-0003-drive-this-by-max_locks_per_transaction.patch | patch -p1
										cat $PATCHES/v20240905-0004-separate-guc-to-allow-benchmarking.patch | patch -p1
								...
								if [ "$groups" != "0" ]; then
									echo "fastpath_lock_groups = $groups" >> $DATADIR/postgresql.auto.conf
								fi
